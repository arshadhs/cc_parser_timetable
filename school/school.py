...
#            print("output[first_name].get('reference') = " + output[first_name].get('reference'))
        ...
        if position is not None:
            list_of_lists[position] = [first_name, output[first_name].get('reference'), 
                                        list_of_lists[position][2]+total9, list_of_lists[position][3]+total10, list_of_lists[position][4]+total11, list_of_lists[position][5]+total12, 
                                        list_of_lists[position][6]+total1, list_of_lists[position][7]+total2, list_of_lists[position][8]+total3, list_of_lists[position][9]+total4, 
                                        list_of_lists[position][10]+total5, list_of_lists[position][11]+total6, list_of_lists[position][12]+total7, list_of_lists[position][13]+total8]
        else:
            masterdata = [first_name, output[first_name].get('reference'), total9, total10, total11, total12, total1, total2, total3, total4, total5, total6, total7, total8]
            list_of_lists.append(masterdata)

#        print("masterData: ", masterData)
#        print (type(first_name))
#        print(type(total1))
        # Map the reference from reference data to the bank st reference, and fetch the particular row
        # for col, x in enumerate((first_name, output[first_name].get('reference'), total9, total10, total11, total12, total1, total2, total3, total4, total5, total6, total7, total8), start=1):
            # if isinstance(x, datetime.datetime):    # If x is Date Time
# #                print("XYZ: ", x)
                # ws.cell(row, col).value = x.strftime('%d-%b-%Y')
                # ws.cell(row, col).alignment = Alignment(horizontal='right')
            # else:                                   # Print anything else in the row
# #                print("abc: ", row, col, x)
                # if (x == 0):
                    # ws.cell(row, col).font = Font(bold=True, color='000000')
                    # ws.cell(row, col).fill = PatternFill("solid", fgColor="D3D3D3")
...
if __name__ == '__main__':
    ...
    start = datetime.date(int(_start), 9, 1)#4, 5)
    end = datetime.date(int(_start) + 1, 8, 31)#4, 4)
    print(start)
    print(end)
