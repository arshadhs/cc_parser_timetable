...
def process_BankAccSt(file_BankAccSt, start, end):
    print(f"Opening file '{file_BankAccSt}'")
    iwb = load_workbook(file_BankAccSt, read_only=True)
    donors = {}
...
# C:\GA>gift_aid.py NatWest-download-2022-23.xlsx GA.xlsx 2022

if __name__ == '__main__':
    file_BankAccSt, file_GA, _start = sys.argv[1:]
    start = datetime.date(int(_start), 4, 5)
    end = datetime.date(int(_start) + 1, 4, 4)
    gift_aid_data = process_giftaid_data(file_GA)
    donors = process_BankAccSt(file_BankAccSt, start, end)
